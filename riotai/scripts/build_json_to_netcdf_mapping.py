"""Rebuild the json_to_netcdf mapping from kerchunk JSON references.

This script is meant to be run directly in a Python session or as a plain
script. It reuses the helper functions in ``riotai.mapping`` and always writes
fresh mapping files instead of loading cached ones.
...
def rebuild_mappings(
    ...
    """Rebuild and overwrite the json_to_netcdf mapping files."""
    ...
    for index, (frequency, frequency_json_paths) in enumerate(
        ...
        logger.info(
            "%d/%d -- Rebuilding frequency '%s' from %d JSON files",
            index,
            len(freq_to_jsons),
            frequency,
            len(frequency_json_paths),
        )
        freq_to_json_to_netcdf[frequency], frequency_errors = _get_netcdf_paths_by_json(
            frequency_json_paths
        )
    ...
    logger.info(
        "Summary | frequencies=%d | jsons=%d | netcdf_paths=%d | errors=%d",
